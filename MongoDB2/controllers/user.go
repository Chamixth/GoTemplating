...
func (uc *UserController) SetupHandler(c echo.Context) error {
	params := new(CrudParams)
	if err := c.Bind(params); err != nil {
		return c.String(http.StatusBadRequest, "Invalid request")
	}

	err := executeTemplate("templates/model.txt", "modelTemplate", params)
	if err != nil {
		return c.String(http.StatusInternalServerError, "Failed to generate model template")
	}

	err = executeTemplate("templates/main.txt", "methodTemplate", params)
	if err != nil {
		return c.String(http.StatusInternalServerError, "Failed to generate methods template")
	}

	err = executeTemplate("templates/method.txt", "mainTemplate", params)
	if err != nil {
